...
def realRel(node,tree):
    if node['relation'] == 'AuxC' or node['relation'] == 'AuxP':
        return realNode(node,tree)['relation']
        ...
        for c in n_c:
            ...
            if rel.endswith('_CO'):
                return rel
        return realRel(n_c[0],tree)
    else:
        return node['relation']
    
def realNode(node,tree):
    if node['relation'] == 'AuxC' or node['relation'] == 'AuxP':
        nodes = []
        for c in children(node,tree):
            if not c['relation'] in ['AuxY','AuxZ','AuxG','AuxX','AuxK','PUNCT','MWE','MWE2']:
                nodes.append(realNode(c,tree))
        if len(nodes) == 1:
            return nodes[0]
        elif len(nodes) == 0:
            id = node['wordid']
