    ...
    def regularize_lemma(self,lemma,form):
        ...
        if re.match('.*-.*',lemma):
            ...
            for i in range(len(splittedPrefix)-1):
                if re.match('.*[aehiouw]',splittedPrefix[i]) and not splittedPrefix[i] == 'peri' and not splittedPrefix[i] == 'pro' and not splittedPrefix[i] == 'a)mfi' and not re.match('[aehiouw].*',splittedPrefix[i+1]):
